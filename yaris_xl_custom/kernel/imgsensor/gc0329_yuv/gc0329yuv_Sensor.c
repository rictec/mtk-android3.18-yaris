...
void GC0329_night_mode(kal_bool bEnable)
{
	...
	else 
	{
		GC0329_SET_PAGE1;
		GC0329_write_cmos_sensor(0x11, 0xa1);
		GC0329_write_cmos_sensor(0x13, 0x7c);
		if(GC0329_MPEG4_encode_mode == KAL_TRUE)
			{
			GC0329_SET_PAGE0;
			GC0329_write_cmos_sensor(0xfa, 0x00);
		...
		GC0329_write_cmos_sensor(0xd5, 0x08);
		GC0329_write_cmos_sensor(0xde, 0x30);//2013.01.29
		GC0329_GAMMA_Select(GC0329_RGB_Gamma_m3);		//GC0329_RGB_Gamma_m1 20130606
	}
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x04, 0x8a);//3
	...
	GC0329_write_cmos_sensor(0x46, 0x86); //02
	...
	GC0329_write_cmos_sensor(0xd1, 0x28); 
	GC0329_write_cmos_sensor(0xd2, 0x28);
	GC0329_write_cmos_sensor(0xd3, 0x3c);	// 40  20130606
	...
	GC0329_write_cmos_sensor(0x13, 0x7c); //50 40
	...
	GC0329_write_cmos_sensor(0x53, 0x1c); //1d,20
	GC0329_write_cmos_sensor(0x54, 0x38); //16,40
	...
	GC0329_write_cmos_sensor(0x5c, 0x03);//3,37
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->PreviewDelayFrame = 5; // 12
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0329_write_cmos_sensor(0x07, 0x00);
			GC0329_write_cmos_sensor(0x08, 0xa2);
			
			GC0329_SET_PAGE1;
			GC0329_write_cmos_sensor(0x29, 0x00);   //anti-flicker step [11:8]
			GC0329_write_cmos_sensor(0x2a, 0x82);   //anti-flicker step [7:0]
				
			GC0329_write_cmos_sensor(0x2b, 0x02);   //exp level 0  30.00fps
			GC0329_write_cmos_sensor(0x2c, 0x8a); 
			GC0329_write_cmos_sensor(0x2d, 0x05);   //exp level 1  14.29fps
			GC0329_write_cmos_sensor(0x2e, 0x14); 
